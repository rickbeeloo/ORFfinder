...
/**
 * Deze class is veranwtoordelijk voor het afhandelen van alle acties die teweeg
 * worden gebracht door de gebruiker.
 *
...
public class ActionHandler {
     ...
     *
     * @param GUISeqWrapper Een sequentie wrapper waarin alle data rondom de
     * ingeladen DNA sequentie in opgeslagen moet worden.
     */
    ...
    /**
     * Deze methode wordt gebruikt voor het maken van een referentie naar een
     * textArea waarin vervolgens alle resultaten getoond kunnen worden.
     *
     * @param outputTextArea Een textArea waarin alle resultaten van de acties
    ...
    /**
     * Deze methode zet de manager die gebruikt moet worden voor het afhandelen
     * van alle taken die betrekking hebben op het BLASTen.
     *
     ...
     * opgeslagen in de database of niet.
     *
     * @param save
     */
    public static void setSaveStatus(Boolean save) {
        saveInDatabase = save;
    }
    ...
    /**
     * Deze methode is nodig voor het extraheren van alle informatie uit het
     * FASTA bestand dat door de gebruiker wordt ingeladen. De methode zal eerst
     * zorgen voor het parse van alle inforamtie waarna de DNA seqentie wordt
     * vertaald in alle 6 de reading frames.
     */
    ...
    /**
     * Deze methode roept alle methodes aan die nodig zijn voor het zoeken van
     * ORF's (rekening houdend met de door de gebruiker ingegeven minimale ORF
     * lengte) en het markeren van deze ORF's in de referentie TextArea.
     */
    public void ORFAction() {
        int minORFLen = showNumbInputDialog("Minimal ORF length: ", "INPUT");
        searchORFs(minORFLen);
        highLightORFs();
    }

    /**
     * Deze methode start een Thread die elke 20 minuten controleerd of er al
    ...
    /**
     * Deze methode bouwt een input formulier waarin de gebruiker de gewenste
     * BLAST parameters kan instellen
     *
    ...
    /**
     * Deze methode verzend een request naar de NCBI server en geeft de
     * informatie dan door aan de BLAST job manager.
     *
     * @param ORFobj Een ORFSequence object waarvan de sequentie geblast moet
     * worden.
     ...
     */
    private int showNumbInputDialog(String txt, String title) {
        String inputValue = JOptionPane.showInputDialog(null, txt, title, JOptionPane.INFORMATION_MESSAGE);
        int minInputLen = 0;
        if (inputValue != null) {
            try {
    ...
    /**
     * @return Deze methode retouneert of de gebruiker heeft aangegeven dat de
     * data niet opgeslagen moet worden in de database.
     */
